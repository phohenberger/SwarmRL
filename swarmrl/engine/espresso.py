...
class EspressoMD(Engine):
    ...
    def add_and_warmup_lattice_boltzmann(self,
        ...
        self.system.integrator.run(n_warmup_timesteps, reuse_forces=True, recalc_forces=False)
    ...
    def _init_h5_output(self):
        ...
        with h5py.File(self.h5_filename.as_posix(), "a") as h5_outfile:
            ...
            if self.system.lb is not None and self.save_LB:
                ...
                self.LB_points = np.array(np.meshgrid(xx, yy, zz))
                self.LB_points = self.LB_points.T.reshape(-1,3)
    ...
    def _update_traj_holder(self):
        ...
        if self.system.lb is not None and self.save_LB:
            ...
            # Black magic to make the LB np.array shape match those generated by np.meshgrid
            # np.meshgrid does weird indexing where the first input get used as the second axis
            lb_vel = np.swapaxes(lb_vel, 0, 3) # simply switch the array containing the 3-dim vector to the beginning
            lb_vel = np.swapaxes(lb_vel, 2, 3) # switch x with z so for [yy, xx, zz]
            
            self.traj_holder["LB_Velocities"].append(
                lb_vel.T.reshape(-1, 3)
            )
    ...
    def manage_forces(self, force_model: ForceFunction = None) -> bool:
        ...
        if force_model is not None:
            ...
            for col in self.colloids:
                if self.lbf is not None:
                    if self.n_dims==2:
                        ...
                        f1 = self.system.lb.get_interpolated_velocity(pos=(col.pos + 1 * direction + 3 * ortho_dir))
                        f2 = self.system.lb.get_interpolated_velocity(pos=(col.pos - 1 * direction + 3 * ortho_dir))
                        f3 = self.system.lb.get_interpolated_velocity(pos=(col.pos + 1 * direction - 3 * ortho_dir))
                        f4 = self.system.lb.get_interpolated_velocity(pos=(col.pos - 1 * direction - 3 * ortho_dir))

                        flow_velocity = np.mean([f1, f2, f3, f4], axis=0)
            ...
            for action, coll in zip(actions, self.colloids):
                ...
                if action.random_torque is True:

                    random_vector = np.random.rand(3)                   
                    coll.ext_torque = random_vector * action.torque
                else:
                    coll.ext_torque = (
                        action.torque
    ...
    def integrate(self, n_slices, force_model: ForceFunction = None):
        ...
        while self.step_idx < self.params.steps_per_slice * (old_slice_idx + n_slices):
            ...
            """for i in range(steps_to_next):
                ...
                print("force ", self.system.part.by_id(1).f)
                print("vel ",self.system.lb.get_interpolated_velocity(pos=self.system.part.by_id(1).pos))
                print("force ", self.system.part.by_id(2).f)
                print("vel ",self.system.lb.get_interpolated_velocity(pos=self.system.part.by_id(2).pos))
                print("force ", self.system.part.by_id(3).f)
                print("vel ",self.system.lb.get_interpolated_velocity(pos=self.system.part.by_id(3).pos))
                print("force ", self.system.part.by_id(4).f)
                print("vel ",self.system.lb.get_interpolated_velocity(pos=self.system.part.by_id(4).pos))
